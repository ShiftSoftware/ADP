...
internal class ClaimableItemsRoutePrefixConvention : IApplicationModelConvention
{
    private readonly AttributeRouteModel prefix;
    private readonly string apiAssemblyName;

    public ClaimableItemsRoutePrefixConvention(string routePrefix)
    {
        prefix = new AttributeRouteModel(new RouteAttribute(routePrefix));
        apiAssemblyName = typeof(ClaimableItemsRoutePrefixConvention).Assembly.GetName().Name!;
    }
