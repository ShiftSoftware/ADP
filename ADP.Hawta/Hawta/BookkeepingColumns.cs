...
/// <c>_PrimaryKey</c>/<c>_RowHash</c> (and optionally a source-modified timestamp) in staging;
/// <see cref="SnapshotMerge"/> owns every other column. Replication columns carry the same
/// semantics as ShiftEntity's <c>IShiftEntityReplication</c>: <c>_LastReplicationDate</c> is the
/// <c>_LastModified</c> of the row version that was pushed — never wall clock.
/// </summary>
public static class BookkeepingColumns
{
    ...
    [
        PrimaryKey, RowHash, SourceScope, LastModified, Deleted, DeletedAt,
