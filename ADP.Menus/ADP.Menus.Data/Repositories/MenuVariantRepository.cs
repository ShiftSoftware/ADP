...
// The maps — the view's soft-delete filters and the write map's reconciliation of the four child
// collections — are in Mappers/MenuMapper.cs.
public class MenuVariantRepository : ShiftRepository<ShiftDbContext, MenuVariant, MenuVariantListDTO, MenuVariantDTO>
{
    ...
    ///
    /// The write map syncs these collections by REMOVING entries the DTO no longer carries. Each
    ...
    /// comment on <c>Items</c> in <c>MenuMapper.ReconcileVariantChildren</c>, which describes this same trap.
    ///
