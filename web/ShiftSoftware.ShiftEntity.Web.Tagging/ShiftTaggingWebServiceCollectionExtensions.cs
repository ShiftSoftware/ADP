...
public static class ShiftTaggingWebServiceCollectionExtensions
{
	...
	/// <para>
	/// The tree only needs the generic parameter because a <see cref="T:ShiftSoftware.TypeAuth.Core.Actions.ReadWriteDeleteAction" /> node
	/// instance doesn't carry its declaring tree type. Registering the same tree again elsewhere is
	/// harmless — <see cref="M:ShiftSoftware.TypeAuth.AspNetCore.TypeAuthAspNetCoreOptions.AddActionTree(System.Type)" /> is idempotent —
	...
	/// </summary>
	public static IServiceCollection AddShiftTagging<TDbContext, TActionTree>(this IServiceCollection services, ReadWriteDeleteAction action) where TDbContext : ShiftDbContext where TActionTree : class
	{
		ShiftTaggingServiceCollectionExtensions.AddShiftTagging<TDbContext>(services, action);
		services.Configure(delegate(TypeAuthAspNetCoreOptions o)
		{
			o.AddActionTree<TActionTree>();
		});
		return services;
	}
}
