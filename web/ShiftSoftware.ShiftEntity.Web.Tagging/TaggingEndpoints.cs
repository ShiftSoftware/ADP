...
public static class TaggingEndpoints
{
	...
	///
	/// Does nothing when <see cref="P:ShiftSoftware.ShiftEntity.EFCore.Tagging.ShiftTaggingOptions.SkipEndpointRegistration" /> is set,
	/// or when <c>AddShiftTagging</c> was not called.
	/// </remarks>
	public static IEndpointRouteBuilder MapShiftTaggingEndpoints<TDbContext>(this IEndpointRouteBuilder endpoints) where TDbContext : ShiftDbContext
	{
		ShiftTaggingOptions val = endpoints.ServiceProvider.GetService<IOptions<ShiftTaggingOptions>>()?.Value;
		if (val == null || !val.Enabled || val.SkipEndpointRegistration)
		{
			return endpoints;
		}
		if (val.Action != null)
		{
			endpoints.MapShiftEntitySecureCrud<ShiftTagRepository<TDbContext>, Tag, TagListDTO, TagDTO>(val.EndpointPrefix, val.Action);
		}
		else
		{
			endpoints.MapShiftEntityCrud<ShiftTagRepository<TDbContext>, Tag, TagListDTO, TagDTO>(val.EndpointPrefix);
		}
		return endpoints;
	}
}
