...
4. **Sample.API running** on `http://localhost:5134` — `cd samples/ADP.Surveys.Sample.API && dotnet run`. The harness logs in as `SuperUser` / `OneTwo` and writes via authenticated CRUD. Set `SURVEYS_API=http://localhost:5135` (or wherever) to drive a second instance instead — handy when the one on :5134 is a debugger session you don't want to restart: `dotnet build samples/ADP.Surveys.Sample.API -o <scratch dir>` then `dotnet <scratch dir>/ADP.Surveys.Sample.API.dll --urls http://localhost:5135 --contentRoot <scratch dir> --environment Development`.
...
| `npm run browser` | `browser.ts` | Same seed, but drives the standalone React app in headless Chromium. Walks Welcome → Feedback (NPS) → Brand, then Back twice (the NPS pick survives, no Back on the first screen) and forward again → Toyota → auto-submit, screenshots each screen to `C:/tmp/survey-renderer-shots/browser/`, verifies DB persistence + zero console errors. The canary for anything that only breaks in a real browser (CSS missing, fetch binding, postMessage, React strict-mode side-effects). |
| `npm run builder-reorder` | `builder-reorder.ts` | Drives the **Blazor builder** in headless Chromium: seeds a survey, flips the form to Edit, then reorders options / questions / screens and switches the selected screen, asserting that every editor shows the data of the object it is bound to and that the next keystroke lands on the right option. Regression for the 2026-09-17 reorder-corruption bug (ShiftBlazor `LocalizedInput` reads `Value` only on init; lists rendered without `@key`). Same puppeteer prerequisites as `browser.ts`; needs no standalone app. |
