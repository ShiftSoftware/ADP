...
??? note "Rule: Availability is scoped to the requester's stock locations"
...
??? note "Rule: The feature has a global on/off switch (EnableSSCPartAvailability)"
...
??? note "Rule: A requester with no stock scope never sees availability"
...
??? note "Rule: Recall part numbers are mapped to the deployment's stored key"
...
??? note "Rule: The default transform ignores dashes and case"
...
??? note "Rule: Availability is only evaluated for recalls that are not yet repaired"
