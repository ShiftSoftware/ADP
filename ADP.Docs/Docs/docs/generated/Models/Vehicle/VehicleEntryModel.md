...
| Intermediary <div><strong>``IntermediarySaleLeg``</strong></div> | The embedded intermediary supply-chain leg, for sources that emit a single entry per VIN and carry the chain inline. Null for multi-entry sources, which instead emit a separate [VehicleEntryModel](/generated/Models/Vehicle/VehicleEntryModel.html) per supply-chain leg. |
| Distributor <div><strong>``DistributorSaleLeg``</strong></div> | The embedded distributor supply-chain leg (the distributor's own sale to the dealer), for single-entry sources on a direct route (distributor→dealer). Null otherwise. The distributor's company comes from `LookupOptions.DistributorCompanyID`; this carries only its invoice number + date. |
