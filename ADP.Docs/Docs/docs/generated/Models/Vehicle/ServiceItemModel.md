...
| EligibilityConditions <div><strong>``IEnumerable<ServiceItemEligibilityConditionModel>``</strong></div> | Optional declarative predicates that must match the vehicle lookup data before this service item is offered. This is a closed contract, not a general query language: the lookup evaluator supports only documented field paths and fails closed for every other path. |
