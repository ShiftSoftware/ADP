...
| IsAvailable <div><strong>``bool?``</strong></div> | Whether this part is currently in stock for the requester: `true` = in stock, `false` = not in
 stock, `null` = availability was not checked (the requester has no Hub stock scope, or the recall is
