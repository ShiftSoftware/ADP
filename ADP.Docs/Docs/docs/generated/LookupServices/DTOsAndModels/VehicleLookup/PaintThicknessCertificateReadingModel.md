...
| PanelLabel <div><strong>``string``</strong></div> | A human-readable label for this panel, e.g. "Door (Front Left)" — derived from the panel's type/position/side `[Description]` attributes. Convenience for the print template and landing page. |
