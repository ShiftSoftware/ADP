...
| Found | A menu was found for the derived code. `Services` can still be empty: a menu whose every variant is deleted, or whose intervals carry no labour details, exists and generates nothing — a different thing from having no menu, and usually rendered differently. |
