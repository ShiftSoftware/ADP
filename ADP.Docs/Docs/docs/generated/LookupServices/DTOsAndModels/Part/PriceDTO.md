...
| UnitPrices <div><strong>``IEnumerable<PartUnitPriceDTO>``</strong></div> | The price broken down by selling unit (e.g. each, box). Only populated for retail prices. Unit names are unique and exactly one entry is marked as the default. |
